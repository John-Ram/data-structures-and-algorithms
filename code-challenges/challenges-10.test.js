...
const createServer = () => {
  ...
  app.get('/aboutme', aboutmeHandler);
    function aboutmeHandler(request, response){
     response.send('I have no idea what i\'m doing!')
  };

  app.get('/favoritefoods', favoritefoodHandler);
    function favoritefoodHandler(request, response){
      response.send(['steak', 'lasanga', 'chorizo with potatoes']);
  }

  app.get('*', status404);
    function status404(request, response){
...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((accum, eachArr) =>{
    const FoundNumsArr = eachArr.filter(val => target === val);
    return accum + FoundNumsArr.length;
  }, 0);
};
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((accum, val) => {
    return accum + val.reduce((innerAccum, innerVal) => innerAccum + innerVal, 0);
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return (input.map(arr => arr.filter(numIsDivIs => {
    if (typeof numIsDivIs === 'number'){
      return numIsDivIs % 5 === 0;
    } else {
      return false;
    }
  }).map(pow => Math.pow(2, pow))));
};
