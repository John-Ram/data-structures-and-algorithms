...
const templateWithJQuery = () => {
  ...
  starWarsPeople.forEach(character => {
    ...
    $htmlTemplate.find('h2').text(character.name);
    $htmlTemplate.find('h3').text(character.height);
    $htmlTemplate.find('p').text(character.eye_color);
...
const listFoods = (recipe) => {
  ...
  gruffaloCrumble.ingredients.forEach(ingredient => {
    ingredient = ingredient.slice(ingredient.indexOf(' ') + 1, ingredient.length);
    ingredient = ingredient.slice(ingredient.indexOf(' ') + 1, ingredient.length);
