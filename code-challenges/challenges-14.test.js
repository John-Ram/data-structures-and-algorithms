...
const createServer = () => {
  ...
  app.delete('/things/1', (request, response) => response.status(405).send());
  // app.get('*', (response, request) => response.status(404).send());
...
let biggerThanLuke = (arr) => {
  ...
  if (luke) {  
    const lukeMass = luke['mass'];
    const bigBois = arr.filter(character => parseInt(character.mass) > parseInt(lukeMass));
    const names = bigBois.map(bigBoi => bigBoi.name);
    result = names.join(' - ');
  }
