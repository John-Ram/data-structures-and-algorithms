...
const templateWithJQuery = () => {
  // Solution code here...
  const sectionClone = $('section:first-child').clone();
  for(let i = 0; i < starWarsPeople.length; i++){
  sectionClone.find('h2').text(starWarsPeople[i].name);
  sectionClone.find('h3').text(starWarsPeople[i].height);
  sectionClone.find('p').text(starWarsPeople[i].eye_color);
...
const listFoods = (recipe) => {
  ...
  for(let i = 0; i < recipe.ingredients.length; i++){
    let ingredientString = recipe.ingredients[i];
    let spaceIndex = ingredientString.indexOf(' ');
    ingredientString = ingredientString.slice(spaceIndex+1);
    spaceIndex = ingredientString.indexOf(' ');
    ingredientString = ingredientString.slice(spaceIndex+1);
